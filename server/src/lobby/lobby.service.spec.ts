...
describe("LobbyService", () => {
    ...
    it("should create a lobby with valid data", () => {
        const result = service.createLobby({ name: "test", password: "pw" });
    ...
    it("should allow same name with different password", () => {
        ...
        expect(() =>
            service.createLobby({ name: "test", password: "pw2" }),
        ).not.toThrow();
    });

    it("should throw BadRequestException for non-string name or password", () => {
        expect(() =>
            service.createLobby({ name: 123 as any, password: "pw" }),
        ).toThrow(BadRequestException);
        expect(() =>
            service.createLobby({ name: "test", password: 123 as any }),
        ).toThrow(BadRequestException);
    });

    it("should list all lobbies without passwords", () => {
        service.createLobby({ name: "lobby1", password: "pw1" });
        service.createLobby({ name: "lobby2", password: "pw2" });
