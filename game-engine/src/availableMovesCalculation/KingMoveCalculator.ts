...
export class KingMoveCalculator implements PieceMoveCalculator {
    ...
    private calculateNonCastlingMoves(
        ...
        const relativeMovesInsideTheBoard = relativeMoves
            .map((relativeMove) => {
                const newRank = addToRank(
                    king.position.rank,
                    relativeMove.rankIncrement
                );
                const newFile = addToFile(
                    king.position.file,
                    relativeMove.fileIncrement
                );
        ...
        const possibleMoves = relativeMovesInsideTheBoard.filter((move) => {
            ...
            return (
                square === null || square.player === negatePlayer(king.player)
            );
        });
    ...
    private calculateCastlingMoves(
        ...
        const movableRooks = getPlayerPieces(board, king.player).filter(
            (piece) => piece.type === PieceType.ROOK && !piece.hasMoved
        );
        const movableRooksWithNoPieceBetweenKing = movableRooks.filter(
            (rook) => {
                const rookFile = rook.position.file;
                const kingFile = king.position.file;

                const filesBetweenKingAndRook = getFileRange(
                    rookFile,
                    kingFile,
                    { inclusive: false }
                );
                const isEverySquareInBetweenEmpty: boolean =
                    filesBetweenKingAndRook.every(
                        (file) => board[file][king.position.rank] === null
                    );
